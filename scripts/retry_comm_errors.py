"""
「通信エラー」のレコードだけを再処理するスクリプト

geocoder_chunked.py で3回リトライしても失敗した通信エラー
（502 Bad Gateway、HTML返却等）を再度APIに問い合わせる。
成功すれば該当住所・緯度・経度・エラーを上書き更新する。

使い方:
    python retry_comm_errors.py [データディレクトリ] [パート数] [同時接続数]
        ...
        for attempt in range(retry_count):
            try:
                async with session.get(
                    ...
                    # Content-Typeチェック（HTML返却対策）
                    content_type = resp.headers.get('Content-Type', '')
                    if 'json' not in content_type and 'text/html' in content_type:
                        if attempt < retry_count - 1:
                            await asyncio.sleep(1.0 * (attempt + 1))
                            continue
                        return {'success': False, 'error': f'通信エラー: HTML返却 (Content-Type: {content_type})'}
...
async def main(output_dir: Path, total_parts: int, max_concurrent: int):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        for part in range(1, total_parts + 1):
            ...
            # 「通信エラー」を含むレコードを対象
            mask = df['エラー'].fillna('').str.contains('通信エラー', na=False)
            ...
            print(f'パート{part}: 通信エラー {len(error_indices)}件をリトライ中...')
    ...
    print(f'\n=== 完了 ===')
    print(f'通信エラー総数: {total_targets}件')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("使い方: python retry_comm_errors.py [データディレクトリ] [パート数] [同時接続数]")
        print("例: python retry_comm_errors.py . 57 500")
        sys.exit(1)
